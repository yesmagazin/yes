...
//        $('.pagination').before('<div id="showmore" class="catalog__btns"><button class="catalog__btn btn btn_orange" onclick="showmore();"><span class="btn__text" style="margin: 12px 2px 12px 2 px;">ПОКАЗАТЬ ЕЩЕ</span></button></div>');
        $('.pagination').after('<div id="showmore" class="catalog__btns"><button class="catalog__btn btn btn_orange" onclick="showmore();"><span class="btn__text" style="margin: 12px 2px 12px 2px;">ПОКАЗАТЬ ЕЩЕ</span></button></div>');
    }
});
function showmore() {
    if ( $( '.category_215' ).length > 0 ) {
        _mgq.push(["MgSensorInvoke", "more646237"]);
    }
