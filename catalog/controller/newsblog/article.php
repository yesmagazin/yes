...
class ControllerNewsBlogArticle extends Controller {
    ...
    public function index() {
        ...
        if ($article_info = $this->model_newsblog_article->getArticle($newsblog_article_id)) {
            ...
            if($categories = $this->model_newsblog_category->getCategories()){
                $data['categories'] = array();
                foreach($categories as $cat){
                    $data['categories'][] = array(
                        'name' => trim($cat['name']),
                        'href' => $this->url->link('newsblog/category', '&newsblog_path=' . $cat['category_id']),
                    );
                }
            } else {
            ...
            $data['heading_title_article'] = $article_info['name'];
            
            $data['text_share'] = $this->language->get('text_share');
            $data['text_categories'] = $this->language->get('text_categories');
            $data['text_relative'] = $this->language->get('text_relative');
            ...
            //form
            if ( $newsblog_article_id == 109 ) {
                $data['text_form_name']     = $this->language->get('text_form_name');
                $data['text_form_mail']     = $this->language->get('text_form_mail');
                $data['text_form_phone']    = $this->language->get('text_form_phone');
                $data['text_form_submit']   = $this->language->get('text_form_submit');
                $data['text_form_descr']    = $this->language->get('text_form_descr');
                $data['text_form_title']    = $this->language->get('text_form_title');

                $data['subscribe_form']     = $this->load->view('newsblog/subscribe.tpl', $data);

                if ( !empty( $this->request->post['subscribe'] ) ) {
                    $formFile = DIR_ROOT . 'subscribeform.json';
                    $formData = file_get_contents( $formFile );
                    $formData = json_decode( $formData, true );
                    $existsMail = false;
                    foreach($formData as $formItem) {
                        if ( $formItem['mail'] == $this->request->post['subscribe']['mail'] ) {
                            $existsMail = true;
                            break;
                        }
                    }
                    if ( !$existsMail && empty($this->request->post['subscribe']['uname']) ) {
                        $formData[] = array(
                            'date'  => date( 'd.m.Y H:i:s' ),
                            'name'  => $this->request->post['subscribe']['name'],
                            'mail'  => $this->request->post['subscribe']['mail'],
                            'phone' => $this->request->post['subscribe']['phone'],
                        );
                        $formData = json_encode($formData);
                        file_put_contents( $formFile, $formData );

                        $data['subscribe_form_success'] = $this->language->get('text_form_success');
                    }
                }
            }
            ...
            $data['date'] = $date_format ? strtr(date($date_format, strtotime($article_info['date_available'])),$months_replace) : false;
            $data['date_modified'] = $date_format ? strtr(date($date_format, strtotime($article_info['date_modified'])),$months_replace) : false;

            $data['viewed'] = $article_info['viewed'];

            $data['articles'] = array();

            $results = $this->model_newsblog_article->getArticlecRelatedCategory($mainCategoryId);
            ...
            foreach ($results as $result) {
                ...
                if ($result['image']) {
                    ...
                    $thumb = $this->model_tool_image->resize($result['image'], 385, 209);
                } else {
                    $original = false;
                    $thumb = false;
                }
                ...
                if(mb_strlen( $preview, 'utf-8' ) > 150){
                    $preview = mb_substr(strip_tags($preview), 0, 150, 'utf-8').'..';
                }
                ...
                $data['articles'][] = array(
                    ...
                    'original'			=> $original,
                    'thumb'       		=> $thumb,
                    'art_name'        	=> $result['art_name'],
                    'cat_name'        	=> $result['cat_name'],
                    'preview'     		=> html_entity_decode($preview, ENT_QUOTES, 'UTF-8'),
                    'href'        		=> $this->url->link('newsblog/article', 'newsblog_path=' . $result['main_category'] . '&newsblog_article_id=' . $result['article_id']),
                    'date'		  		=> ($date_format ? strtr(date($date_format, strtotime($result['date_available'])),$months_replace) : false),
                );
            }
            ...
            $template_default = $settings && $settings['template_article'] ? $settings['template_article'] : 'article.tpl';
