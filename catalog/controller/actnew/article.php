<?php
///*Fulle changed. Based on product/category.php*/
///Fulle changed. Based on actnew/category.php

class ControllerActnewArticle extends Controller {
	public function index() {
        $this->load->language('product/category');
		$this->load->language('actnew/category');
		...
		if (isset($this->request->get['actnew_article_id'])) {
            $category_id = $this->request->get['actnew_article_id'];
        } else {
		if (isset($this->request->get['actnew_path'])) {
			...
			foreach ($parts as $path_id) {
				...
				$category_info = $this->model_actnew_category->getCategory($path_id);

				if ($category_info) {
					$data['breadcrumbs'][] = array(
						'text' => $category_info['name'],
						'href' => $this->url->link('actnew/category', 'actnew_path=' . $path)
					);
				}
			}
		} else {
		...
		$category_info = $this->model_actnew_category->getCategory($category_id);
		if ($category_info) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $category_info['name'],
				'href' => $this->url->link('actnew/category', 'actnew_path=' . $this->request->get['actnew_path'])
			);
			...
			$categories = $this->model_actnew_category->getCategories($category_id);
			...
			foreach ($categories as $category) {
				...
				$articles_total = $this->model_actnew_article->getTotalArticles($filter_data);
				...
				$data['categories'][] = array(
					...
					'original'		=> $original,
					'thumb'			=> $thumb,
					'href' 			=> $this->url->link('actnew/category', 'actnew_path=' . $this->request->get['actnew_path'] . '_' . $category['category_id'])
				);
			}
			...
			if ($limit>0) {
				...
				foreach ($results as $result) {
					...
					$data['articles'][] = array(
						...
						'href'        		=> $this->url->link('actnew/article', 'actnew_path=' . $this->request->get['actnew_path'] . '&actnew_article_id=' . $result['article_id']),
				...
				$pagination->url = $this->url->link('actnew/category', 'actnew_path=' . $this->request->get['actnew_path'] . '&page={page}');
				...
				if ($page == 1) {
				    $this->document->addLink($this->url->link('actnew/category', 'actnew_path=' . $category_info['category_id'], 'SSL'), 'canonical');
				} elseif ($page == 2) {
				    $this->document->addLink($this->url->link('actnew/category', 'actnew_path=' . $category_info['category_id'], 'SSL'), 'prev');
				} else {
				    $this->document->addLink($this->url->link('actnew/category', 'actnew_path=' . $category_info['category_id'] . '&page='. ($page - 1), 'SSL'), 'prev');
				}

				if ($limit && ceil($article_total / $limit) > $page) {
				    $this->document->addLink($this->url->link('actnew/category', 'actnew_path=' . $category_info['category_id'] . '&page='. ($page + 1), 'SSL'), 'next');
				}
			}

			$data['canonical']=$this->url->link('actnew/category', 'actnew_path=' . $category_info['category_id'], 'SSL');
			...
			$this->response->setOutput($this->load->view('actnew/'.$template_default, $data));
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_error'),
				'href' => $this->url->link('actnew/category')
			);
