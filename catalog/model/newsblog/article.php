...
class ModelNewsBlogArticle extends Model {
    ...
    public function getArticle($article_id) {
        $query = $this->db->query("SELECT DISTINCT *, pd.name AS name, pd.description AS description, pd.preview AS preview, p.image, p.image_large,
		p.sort_order, pcd.name AS cat_name, pd.meta_title AS meta_title, pd.meta_description AS meta_description, pd.meta_keyword AS meta_keyword, pd.meta_h1 AS meta_h1
        ...
        LEFT JOIN " . DB_PREFIX . "newsblog_article_to_category pc ON (pc.article_id = p.article_id)
        LEFT JOIN " . DB_PREFIX . "newsblog_category_description pcd ON (pcd.category_id = pc.category_id AND pcd.language_id = pd.language_id)
        ...
        if ($query->num_rows) {
            return array(
                ...
                'cat_name'         => trim($query->row['cat_name']),
                'name'             => trim($query->row['name']),
                'preview'      	   => trim($query->row['preview']),
                ...
                'image_large'      => $query->row['image_large'],
                ...
                'attributes'	   => false //$this->getArticleAttributes($article_id)
            );
        } else {
            return false;
        }
	...
	public function getArticlesIDs($aids) {
        if($aids && is_array($aids)) {
            //$language_id = (int)$this->config->get('config_language_id');
            $query = $this->db->query("SELECT DISTINCT *, pd.name AS name, pcd.name cat_name, p.image, p.sort_order, pc.category_id acid
            FROM " . DB_PREFIX . "newsblog_article p
            LEFT JOIN " . DB_PREFIX . "newsblog_article_description pd ON (p.article_id = pd.article_id)
            LEFT JOIN " . DB_PREFIX . "newsblog_article_to_store p2s ON (p.article_id = p2s.article_id)
            LEFT JOIN " . DB_PREFIX . "newsblog_article_to_category pc ON (pc.article_id = p.article_id)
            LEFT JOIN " . DB_PREFIX . "newsblog_category_description pcd ON (pcd.category_id = pc.category_id AND pcd.language_id = pd.language_id)
            WHERE p.article_id IN (" . implode(',',$aids) . ") AND p.status = '1' AND p.date_available <= NOW() AND
            p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY p.article_id, pd.language_id ORDER BY p.date_added DESC");
            $articles = array();
            if ($query->num_rows) {
                foreach ($query->rows as $row) {
                    $articles[$row['language_id']][] = array(
                        ...
                        'viewed' => $row['viewed'],
                        'attributes' => array() //$this->getArticleAttributes($article_id)
                    );
                }
                return $articles;
            }
    ...
    public function getArticles($data = array(), $header = 0) {
		...
		LEFT JOIN " . DB_PREFIX . "newsblog_category nbc ON (nbc.category_id = a2c.category_id)
        ...
        if(!$header) {
            //$sql .= " AND nbc.show_in_header = " . $header;
        }
        ...
        if (isset($data['sort'])) {
            ...
            } elseif($data['sort'] == 'a.date_available') {
...
//var_dump($sql);
	...
	public function getArticlesMod($data = array()) {

	    $key = md5(json_encode($data));
	    if($articles = $this->cache->get('articles'.$key)){
	        return $articles;
        } else {
            $sql = "SELECT a.article_id";
		...
		a2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
            ...
            if($query->num_rows){
                $aids = array();
                foreach ($query->rows as $result) {
                    $aids[] = $result['article_id'];
                }

                if(!empty($aids)){
                    $articles = $this->getArticlesIDs($aids);
                    $this->cache->set('articles'.$key,$articles);
                    return $articles;
                }
            }
        }
    ...
    public function getArticlecRelatedCategory($category_id) {
        $query = $this->db->query("SELECT *,pd.`name` art_name,pcd.`name` cat_name FROM " . DB_PREFIX . "newsblog_article p LEFT JOIN " . DB_PREFIX . "newsblog_article_description pd ON (p.article_id = pd.article_id) LEFT JOIN " . DB_PREFIX . "newsblog_article_to_category p2c ON (p.article_id = p2c.article_id) LEFT JOIN " . DB_PREFIX . "newsblog_category_description pcd ON (pcd.category_id = p2c.category_id AND pcd.language_id = pd.language_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$category_id . "' AND p.`status` = 1 ORDER BY RAND() LIMIT 3");
    ...
    public function getTotalArticles($data = array(), $header = 0) {
    ...
    public function getTotalArticlesAll($data = array(), $header = 0) {
        $sql = "SELECT COUNT(DISTINCT p.article_id) AS total";

        $sql .= " FROM " . DB_PREFIX . "newsblog_article_to_category p2c";

        $sql .= " LEFT JOIN " . DB_PREFIX . "newsblog_article p ON (p.article_id = p2c.article_id) LEFT JOIN " . DB_PREFIX . "newsblog_category c ON (c.category_id = p2c.category_id)  WHERE  p.status = '1' ";

        if(!$header) {
            $sql .= " AND c.show_in_header = '".$header ."'";
        }
    ...
    public function getArticlesAll($data = array(), $header = 0) {
        $sql = "SELECT a.article_id";

        $sql .= " FROM " . DB_PREFIX . "newsblog_article a LEFT JOIN " . DB_PREFIX . "newsblog_article_to_category a2c ON (a.article_id = a2c.article_id)";
    ...
    public function getPermissionsInfo($article_id) {
        $sql = "SELECT nbc.show_in_header";

        $sql .= " FROM " . DB_PREFIX . "newsblog_category nbc LEFT JOIN " . DB_PREFIX . "newsblog_article_to_category a2c ON (nbc.category_id = a2c.category_id)";

        $sql .= " LEFT JOIN " . DB_PREFIX . "newsblog_article a ON (a.article_id = a2c.article_id) WHERE 

		a.status = '1' AND
		a.date_available <= NOW() AND a.article_id ='" . $article_id . "'";
        $article_data = array();

//        return $sql;
        $query = $this->db->query($sql);
        if($query->row)
            return $query->row['show_in_header'];
        else return 1;
