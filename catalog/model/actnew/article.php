...
class ModelActnewArticle extends Model {
	public function updateViewed($article_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "actnew_article SET viewed = (viewed + 1) WHERE article_id = '" . (int)$article_id . "'");
	}
	...
	public function getArticleCategory($article_id) {
		...
		(select price from " . DB_PREFIX . "actnew_article where article_id='" . (int)$article_id . "') as price,
		(select name from " . DB_PREFIX . "actnew_article_description where article_id='" . (int)$article_id . "') as name,
        (SELECT price FROM " . DB_PREFIX . "actnew_article_special ps WHERE ps.article_id = '" . (int)$article_id . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special

		FROM " . DB_PREFIX . "actnew_article_to_category ptc
	...
	public function getArticle($article_id) {
		...
		LEFT JOIN " . DB_PREFIX . "actnew_article_to_store p2s ON (p.article_id = p2s.article_id)
	...
	public function getArticleBySeourl($upc) {
        static $cat = array();
        ...
        $r = $query->row;
        $found = false;
        if (is_array($r) && !empty($r)) { //[query] => actnew_article_id=1
            if (strstr($r['query'], 'actnew_article_id=')) {
                $found = str_replace('actnew_article_id=', '', $r['query']);
            }
        }
        if ($found) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image,
		...
		LEFT JOIN " . DB_PREFIX . "actnew_article_to_category p2c ON (p.article_id = p2c.article_id)

		WHERE p.article_id = '" . (int)$found . "' AND
		pd.language_id = '" . $lid . "' AND
		p.status = '1' AND
		p.date_available <= NOW() AND
		p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		...
		if ($query->num_rows) {
			$ret = array(
				...
				'date_modified'    => $query->row['date_modified'],
				'viewed'           => $query->row['viewed'],
//				'attributes'	   => $this->getArticleAttributes($found)
			);
            if (!isset($cat[$lid][$query->row['category_id']]) || empty($cat[$lid][$query->row['category_id']])) {
                $query2 = $this->db->query("SELECT DISTINCT c.settings, cd.name AS name
                FROM " . DB_PREFIX . "actnew_category c
                LEFT JOIN " . DB_PREFIX . "actnew_category_description cd ON (c.category_id = cd.category_id)
                WHERE c.category_id = '" . $query->row['category_id'] . "' AND
                cd.language_id = '" . $lid . "'");
                $sett = unserialize($query2->row['settings']);
                $zzz = array(
                    'category_name' => $query2->row['name'],
                    'category_image_width' => $sett['image_size_width'],
                    'category_image_height' => $sett['image_size_height'],
                );
                $cat[$lid][$query->row['category_id']] = $zzz;
            }
            $ret = array_merge($ret, $cat[$lid][$query->row['category_id']]);
            return $ret;
		}
        return false;
	}
	...
	public function getArticles($data = array()) {
		...
		if (!empty($data['filter_category_id']) || !empty($data['filter_categories'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " FROM " . DB_PREFIX . "actnew_category_path cp LEFT JOIN " . DB_PREFIX . "actnew_article_to_category a2c ON (cp.category_id = a2c.category_id)";
			} else {
				$sql .= " FROM " . DB_PREFIX . "actnew_article_to_category a2c";
			}

				$sql .= " LEFT JOIN " . DB_PREFIX . "actnew_article a ON (a2c.article_id = a.article_id)";

		} else {
			$sql .= " FROM " . DB_PREFIX . "actnew_article a";
		}

		$sql .= " LEFT JOIN " . DB_PREFIX . "actnew_article_description ad ON (a.article_id = ad.article_id)
		LEFT JOIN " . DB_PREFIX . "actnew_article_to_store a2s ON (a.article_id = a2s.article_id)
	...
	public function getLatestarticles($limit) {
		...
		if (!$article_data) {
			$query = $this->db->query("SELECT p.article_id FROM " . DB_PREFIX . "actnew_article p LEFT JOIN " . DB_PREFIX . "actnew_article_to_store p2s ON (p.article_id = p2s.article_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getArticleDiscounts($article_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "actnew_article_discount WHERE article_id = '" . (int)$article_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity > 1 AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity ASC, priority ASC, price ASC");
	...
	public function getArticleRelated($article_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "actnew_article_related pr
		LEFT JOIN " . DB_PREFIX . "actnew_article p ON (pr.related_id = p.article_id)
		LEFT JOIN " . DB_PREFIX . "actnew_article_to_store p2s ON (p.article_id = p2s.article_id)
	...
	public function getArticleRelatedProducts($article_id) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "actnew_article_related pr
	...
	public function getArticleLayoutId($article_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "actnew_article_to_layout WHERE article_id = '" . (int)$article_id . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	public function getTotalArticles($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "actnew_article_to_category p2c ON (cp.category_id = p2c.category_id)";
			} else {
				$sql .= " FROM " . DB_PREFIX . "actnew_article_to_category p2c";
			}

			if (!empty($data['filter_filter'])) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "actnew_article_filter pf ON (p2c.article_id = pf.article_id) LEFT JOIN " . DB_PREFIX . "actnew_article p ON (pf.article_id = p.article_id)";
			} else {
				$sql .= " LEFT JOIN " . DB_PREFIX . "actnew_article p ON (p2c.article_id = p.article_id)";
			}
		} else {
			$sql .= " FROM " . DB_PREFIX . "actnew_article p";
		}
	...
	public function getArticleMainCategoryId($article_id) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "actnew_article_to_category WHERE article_id = '" . (int)$article_id . "' order by main_category desc LIMIT 1");
