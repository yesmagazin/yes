...
class ModelLocalisationLocation extends Model {
	...
	public function getLocationsByIDs($location_ids) {
	    if($location_ids && is_array($location_ids)){
            $query = $this->db->query("SELECT location_id, name, address, geocode, telephone, fax, image, open, comment FROM " . DB_PREFIX . "location WHERE location_id IN (" . implode(',',$location_ids) . ")");
