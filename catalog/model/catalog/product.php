...
class ModelCatalogProduct extends Model {
	...
	public function getProductsByIDs($product_ids,$data=array()) {
        ...
        $sort_data = array(
            ...
            'FIELD(p.stock_status_id, 7, 21, 12, 11) ASC, p.product_id',
            ...
            'FIELD(p.stock_status_id, 7, 21, 12, 11) DESC, FIELD(ptc.category_id, 3, 169, 12, 4) DESC, p.product_id',
            'FIELD(p.stock_status_id, 7, 21, 12, 11, 23) DESC, FIELD(ptc.category_id, 3, 169, 12, 4) DESC, p.product_id',
            'FIELD(p.stock_status_id, 7, 21, 12, 11) DESC, FIELD(ptc.category_id, 4, 12, 169, 3) DESC, p.product_id',
            'FIELD(p.stock_status_id, 7, 21, 12, 11, 23) DESC, FIELD(ptc.category_id, 4, 12, 169, 3) DESC, p.product_id',
        );
        $sort .= " ORDER BY ";
        /**
         * Order for actions backpacks
         */
        //$sort .= "p.temp_meta DESC, ";
...
//var_dump($query->rows); exit;
	...
	public function getProductsCalc($data = array()) {
         ...
         * Order for actions backpacks
         */
        //$sql .= "p.temp_meta DESC, ";

//        $sqlBackpackFirst = "SELECT DISTINCT(ptc.product_id) FROM " . DB_PREFIX . "product_to_category ptc ORDER BY FIELD(ptc.category_id, 4, 12, 169, 3) DESC";
        $sqlBackpackFirst = "locate('юкзак', pd.name) DESC, locate('ackpack', pd.name) DESC";
        $pdname = false;
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				$sql .= "LCASE(" . $data['sort'] . ") ";
			    ...
			    $tempArraySort = explode(',', $data['sort']);
			    if($tempArraySort[count($tempArraySort) - 1] == ' p.product_id') {
                    $tempArraySort[count($tempArraySort) - 1] = "{$sqlBackpackFirst}, p.product_id ";
                    if(!empty($data['filter_category_id']) && $data['filter_category_id'] == 2) {
                        $data['sort'] = implode(',', $tempArraySort);
                    }
...
//var_dump($product_ids);exit;
	...
	public function getProductAttributes($product_id, $getFilters = false) {
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			foreach ($product_attribute_query->rows as $product_attribute) {
                ...
                $filtArr = array();
                if ( $getFilters ) {
                    $filt = $this->db->query( "SELECT ov.option_id, ov.value_id FROM " . DB_PREFIX . "ocfilter_option_value_to_product ov LEFT JOIN " . DB_PREFIX . "ocfilter_option_value_description ovd ON (ov.value_id = ovd.value_id) WHERE ov.product_id = '" . (int)$product_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ovd.name = '".$product_attribute['text']."'" );

                    foreach ($filt->rows as $filtItem) {
                        $filtArr[] = array(
                            'option_id' => $filtItem['option_id'],
                            'value_id' => $filtItem['value_id'],
                        );
                    }
                }
    ...
    public function relbuildNewestCategory() {
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE category_id = 209");
//        $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category (product_id, category_id, main_category)
//SELECT p.product_id, 209, 0 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (pc.product_id = p.product_id) WHERE p.product_id LIKE '558%' AND pc.category_id IN (169,12,4,3) GROUP BY p.product_id");
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category (product_id, category_id, main_category)
SELECT p.product_id, 209, 0 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (pc.product_id = p.product_id) WHERE p.stock_status_id IN (11, 23) GROUP BY p.product_id");
        $this->cache->delete('ocfilter');
        $this->cache->delete('product');
    }

    public function relbuildAndretanCategory() {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET stock_status_id = 23 WHERE model IN (557980, 555094, 558306, 557681, 557855, 558784, 557838, 557853, 557833, 557839, 557847, 558610, 557846, 558611, 558800, 556379, 555483, 558580, 555482, 555479, 555480, 555478, 558568, 557364, 558571, 558457, 558217, 558210, 558211, 558213, 558794, 558553, 558521, 558538, 558555, 556835, 558523, 558522, 558557, 558535, 558035, 557997, 557955, 558244, 558566, 558004, 555484, 555487, 554682, 558347, 557994, 554860, 551665, 558293, 551613, 558608, 558432, 557881, 556312, 556313, 556315, 554692, 555526, 555527, 552534, 552527, 558752, 557861, 557863, 558749, 557864, 557868, 557867, 557870, 557841, 557842, 554589, 554689, 554688, 554696, 558319, 558318, 558741, 558745, 558742, 558746, 556251, 556257, 552270, 552274, 552272, 558874, 558875, 558876, 558877, 558878, 558879, 558880, 558881)");
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE category_id = 215");
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category (product_id, category_id, main_category)
SELECT p.product_id, 215, 0 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (pc.product_id = p.product_id) WHERE p.stock_status_id = 23 GROUP BY p.product_id");
        $this->cache->delete('ocfilter');
        $this->cache->delete('product');
    }

    public function relbuildAndretanDesignCategory() {
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE category_id = 216");
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category (product_id, category_id, main_category)
SELECT p.product_id, 216, 0 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (pc.product_id = p.product_id) WHERE p.product_id IN (" . implode(",", $this->productsByAndreTan() ). ") GROUP BY p.product_id");
        $this->cache->delete('ocfilter');
        $this->cache->delete('product');
    }

    public function isBackpack($pid) {
        $query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = {$pid} ");
        if($query->num_rows > 0) {
            foreach( $query->rows as $row ) {
                if( in_array($row['category_id'], array(169,12,4,3)) )
                    return true;
            }
        }
        return false;
    ...
    public function productsByAndreTan() {
        return array(558874, 558875, 558876, 558877, 558878, 558879, 558880, 558881);
    }
}
