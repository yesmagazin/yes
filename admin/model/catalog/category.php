...
class ModelCatalogCategory extends Model {
	...
	public function getCategoriesIDs() {
		$query = $this->db->query("SELECT `category_id` FROM " . DB_PREFIX . "category WHERE `status` = 1 ORDER BY `category_id`");
        if($query->num_rows){
            $cids = array();
            foreach($query->rows as $row){
                $cids[] = (int)$row['category_id'];
            }
            return $cids;
        }
        return false;
    ...
    public function getCategoriesIDsPath() {
        $query = $this->db->query("SELECT c.`category_id`, cp.`path_id`, cp.`level` FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_path cp ON (c.`category_id` = cp.`category_id`) WHERE `status` = 1 ORDER BY c.`category_id` ASC, cp.`level` ASC");
        if($query->num_rows){
            $cids = array();
            $currentCatID = -1;
            foreach($query->rows as $row){
                if ( $currentCatID != $row['category_id'] ) {
                    ...
                    $catPath = array();
                    foreach($query->rows as $rowTemp){
                        if ( $rowTemp['category_id'] == $currentCatID ) {
                            $catPath[] = $rowTemp['path_id'];
                        }
                    }
                    if ( ! empty( $catPath ) ) {
                        $cids[] = implode( '_', $catPath );
                    }
                }
