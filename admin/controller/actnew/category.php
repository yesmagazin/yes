...
//		if (empty($this->session->data['check_version_actnew'])) $this->session->data['check_version_actnew']=@file_get_contents('http://nedorogoi-internet-magazin.ru/check_version.php?now='.$this->version_actnew.'&version='.VERSION.'&site='.$_SERVER['HTTP_HOST']);
		...
		$this->language->load('actnew/category');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_actnew_category->addCategory($this->request->post, $settings);
			...
			$this->response->redirect($this->url->link('actnew/category', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_actnew_category->editCategory($this->request->get['category_id'], $this->request->post, $settings);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $category_id) {
				$this->model_actnew_category->deleteCategory($category_id);
			}
	...
	public function repair() {
		...
		if ($this->validateRepair()) {
			$this->model_actnew_category->repairCategories();

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('actnew/category', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('actnew/category', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('actnew/category/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('actnew/category/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['repair'] = $this->url->link('actnew/category/repair', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$category_total = $this->model_actnew_category->getTotalCategories();

		$results = $this->model_actnew_category->getCategories($filter_data);

		foreach ($results as $result) {
			$data['categories'][] = array(
				'category_id' 		=> $result['category_id'],
				'name'        		=> $result['name'],
				'count_elements' 	=> $this->model_actnew_article->getTotalArticles(array('filter_category'=>$result['category_id'])),
				'sort_order'  		=> $result['sort_order'],
				'edit'        		=> $this->url->link('actnew/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true),
				'delete'      		=> $this->url->link('actnew/category/delete', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('actnew/category', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('actnew/category', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('actnew/category', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('actnew/category_list.tpl', $data));
	}
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('actnew/category/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('actnew/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('actnew/category', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$category_info = $this->model_actnew_category->getCategory($this->request->get['category_id']);
		...
		if (isset($this->request->post['category_description'])) {
			...
			$data['category_description'] = $this->model_actnew_category->getCategoryDescriptions($this->request->get['category_id']);
		} else {
			$data['category_description'] = array();
		}

		// Categories
		$categories = $this->model_actnew_category->getAllCategories();
		...
		if (isset($this->request->post['category_store'])) {
			...
			$data['category_store'] = $this->model_actnew_category->getCategoryStores($this->request->get['category_id']);
		} else {
		...
		if (isset($this->request->post['category_layout'])) {
			...
			$data['category_layout'] = $this->model_actnew_category->getCategoryLayouts($this->request->get['category_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('actnew/category_form.tpl', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'actnew/category')) {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['category_id']) && $url_alias_info['query'] != 'actnew_category_id=' . $this->request->get['category_id']) {
	...
	public function update() {
		...
		$this->model_actnew_category->updateDateBase();
