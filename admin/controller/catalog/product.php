...
class ControllerCatalogProduct extends Controller {
    ...
    protected function getList() {
        ...
        foreach ($results as $result) {
            ...
            if (is_file(DIR_IMAGE . $result['image_hover'])) {
                $image_hover = $this->model_tool_image->resize($result['image_hover'], 40, 40);
            } else {
                $image_hover = $this->model_tool_image->resize('no_image.png', 40, 40);
            }
    ...
    protected function getForm() {
        ...
        $data['entry_infografika'] = $this->language->get('entry_infografika');
        ...
        // Infografika
        if (isset($this->request->post['infografika'])) {
            $data['infografika'] = $this->request->post['infografika'];
        } elseif (!empty($product_info)) {
            $data['infografika'] = $product_info['infografika'];
        } else {
            $data['infografika'] = '';
        }

        if (isset($this->request->post['infografika']) && is_file(DIR_IMAGE . $this->request->post['infografika'])) {
            $data['thumb_inf'] = $this->model_tool_image->resize($this->request->post['infografika'], 100, 100);
        } elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['infografika'])) {
            $data['thumb_inf'] = $this->model_tool_image->resize($product_info['infografika'], 100, 100);
        } else {
            $data['thumb_inf'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        }
    ...
    private function refreshSphinx(){
        @exec('/var/www/top/data/sphinx/bin/indexer --rotate productsyes  -c /var/www/top/data/www/sphinx.conf > /dev/null 2>&1 &');
    }
}
