...
class ControllerStartupSeoPro extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->cache_data) {
			...
			$this->cache->set('seo_pro', $this->cache_data, 86400);
		}
	}
      ...
      // OCFilter end
		...
		if (!isset($this->request->get['_route_'])) {
			...
			if (count($rows) == sizeof($parts)) {
				...
				foreach ($parts as $part) {
					...
					$url = explode('=', $queries[$part], 2);
      				...
      				if ($url[0] == 'actnew_category_id') {
						if (!isset($this->request->get['actnew_path'])) {
							$this->request->get['actnew_path'] = $url[1];
						} else {
							$this->request->get['actnew_path'] .= '_' . $url[1];
						}
					} else
		    ...
		    if (isset($this->request->get['actnew_article_id'])) {
				$this->request->get['route'] = 'actnew/article';
				if (!isset($this->request->get['actnew_path'])) {
					$path = $this->getPathByactnewArticle($this->request->get['actnew_article_id']);
					if ($path) $this->request->get['actnew_path'] = $path;
				}
			} elseif (isset($this->request->get['actnew_path'])) {
				$this->request->get['route'] = 'actnew/category';
			} else
	...
	public function rewrite($link) {
		...
		switch ($route) {
      			...
      			case 'actnew/article':
				if (isset($data['actnew_article_id'])) {
					$tmp = $data;
					$data = array();
					if ($this->config->get('config_seo_url_include_path')) {
						$data['actnew_path'] = $this->getPathByactnewArticle($tmp['actnew_article_id']);
						if (!$data['actnew_path']) return $link;
					}
					$data['actnew_article_id'] = $tmp['actnew_article_id'];
				}
				break;

			case 'actnew/category':
				if (isset($data['actnew_path'])) {
					$category = explode('_', $data['actnew_path']);
					$category = end($category);
					$data['actnew_path'] = $this->getPathByactnewCategory($category);
					if (!$data['actnew_path']) return $link;
				}
				break;
				...
				switch ($key) {
	       			...
	       			case 'actnew_path':
						...
						foreach($categories as $category) {
							$queries[] = 'actnew_category_id=' . $category;
						}
						unset($data[$key]);
						break;

					case 'actnew_article_id':
					case 'actnew_category_id':
	...
	private function getPathByactnewArticle($article_id) {
		$article_id = (int)$article_id;
		if ($article_id < 1) return false;

		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('actnew.article.seopath');
			if (!isset($path)) $path = array();
		}

		if (!isset($path[$article_id])) {
			$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "actnew_article_to_category WHERE article_id = '" . $article_id . "' ORDER BY main_category DESC LIMIT 1");

			$path[$article_id] = $this->getPathByactnewCategory($query->num_rows ? (int)$query->row['category_id'] : 0);

			$this->cache->set('actnew.article.seopath', $path);
		}

		return $path[$article_id];
	}

	private function getPathByactnewCategory($category_id) {
		$category_id = (int)$category_id;
		if ($category_id < 1) return false;

		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('actnew.category.seopath');
			if (!isset($path)) $path = array();
		}
		...
		if (!isset($path[$category_id])) {
			...
			$sql .= ") AS path FROM " . DB_PREFIX . "actnew_category t0";
			for ($i = 1; $i < $max_level; ++$i) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "actnew_category t$i ON (t$i.category_id = t" . ($i-1) . ".parent_id)";
			}
			...
			$this->cache->set('actnew.category.seopath', $path);
		}
