...
class ControllerStartupSeoUrl extends Controller {
		...
		if (isset($this->request->get['_route_'])) {
			...
			$mfilterConfig = $this->config->get( 'mega_filter_seo' );
				...
				if ($query->num_rows) {
					...
					if ($url[0] == 'manufacturer_id') {
      				...
      				if ($url[0] == 'actnew_article_id') {
						$this->request->get['actnew_article_id'] = $url[1];
					}
					...
					if ($query->row['query'] && $url[0] != 'actnew_category_id' && $url[0] != 'actnew_article_id' && $url[0] != 'newsblog_category_id' && $url[0] != 'newsblog_article_id' && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
			...
			if (!isset($this->request->get['route'])) {
				if (isset($this->request->get['product_id'])) {
      			...
      			} elseif (isset($this->request->get['actnew_article_id'])) {
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'actnew/article' && $key == 'actnew_article_id') || ($data['route'] == 'newsblog/article' && $key == 'newsblog_article_id') || ($data['route'] == 'information/information' && $key == 'information_id')) {
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = '" . $this->db->escape($key . '=' . (int)$value) . "' AND `language_id` = 3");
      			...
      			} elseif ($key == 'actnew_path') {
					$categories = explode('_', $value);

					foreach ($categories as $category) {
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = 'actnew_category_id=" . (int)$category . "'");
