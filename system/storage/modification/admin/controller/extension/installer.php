				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						$json['step'][] = array(
							...
							'url'  => str_replace('&amp;', '&', $this->url->link((($this->request->get['route'] == 'extension/installer/do-uninstall') ? 'extension/installer/'. $this->rewrite_step('ftp', 'localremove') : 'extension/installer/localcopy'), 'token=' . $this->session->data['token'], true)),
							'path' => $path
						);
