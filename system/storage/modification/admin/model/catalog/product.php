...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['infografika'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET infografika = '" . $this->db->escape($data['infografika']) . "' WHERE product_id = '" . (int)$product_id . "'");
		}
    ...
    public function getProductsIDs( $getAll = false ) {
	    $where = ( !$getAll ) ? "WHERE `price` > 0 AND `status` = 1" : "";
        $query = $this->db->query("SELECT `product_id` FROM " . DB_PREFIX . "product {$where} ORDER BY `product_id`");
        if($query->num_rows){
            $pids = array();
...
///!!! ALTER TABLE  `orders_products` CHANGE  `products_quantity`  `products_quantity` INT( 8 ) NOT NULL DEFAULT '0';
/*        $this->db->query("ALTER TABLE `" . DB_PREFIX . "product` CHANGE `mpn` `mpn` varchar(64) NOT NULL");
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "product` CHANGE `isbn` `isbn` varchar(64) NOT NULL");
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "product` CHANGE `upc` `upc` varchar(64) NOT NULL");
2020-04-22 Sergey R.  */      
    ...
    public function getProductsIDsByStatusId( $status_id = 0 ) {
        if($status_id){
            $query = $this->db->query("SELECT DISTINCT `product_id` FROM `" . DB_PREFIX . "product` WHERE `stock_status_id` = ".(int)$status_id." AND `status` = 1 ORDER BY RAND() LIMIT 10");
            if($query->rows){
                $data = array();
                foreach($query->rows as $row){
                    $data[] = $row['product_id'];
    ...
    public function getPromoProducts() {
        $query = $this->db->query("SELECT DISTINCT p.`product_id` FROM `" . DB_PREFIX . "promotions_content` pc
                    ...
                    WHERE pr.`active` = 1 AND p.`product_id` IS NOT NULL AND p.`status` = 1 ORDER BY RAND() LIMIT 10");
        if($query->rows){
            $data = array();
            foreach($query->rows as $row){
                $data[] = $row['product_id'];
