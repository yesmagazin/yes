...
class AcImage
{
	...
	private static $maxProportionLogo = 0.2;
	...
	public function save($filePath){
        ...
        {
            case self::JPEG :
                return $this->saveAsJPG($filePath);
            case self::PNG :
                return $this->saveAsPNG($filePath);
            case self::GIF :
                return $this->saveAsGIF($filePath);
            default:
                throw new InvalidFileException($filePath);
        }
    }
	...
	public function saveAsJPG($path)
	{
        ...
        if(!imageinterlace(self::getResource(), 1))
            throw new FileNotInterlacedException($path);
 ...
 /**
	...
	public function resize() // Size $s || $width, $height
	{
		...
		else if (count($args) == 1 && $args[0] instanceof Size)
		{
            ...
            if(isUFO()){
                var_dump($imageSize,$imageSize->getWidth(), $imageSize->getHeight(), $this->getWidth(), $this->getHeight());
                exit;
            }
